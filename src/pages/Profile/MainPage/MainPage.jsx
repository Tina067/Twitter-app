...
const MainPage = ({ user }) => {
  ...
  useEffect(() => {
      fetch(`${window.location.origin}/userPost?email=${user?.email}`)
        .then(res => res.json())
        .then(data => {
          setPosts(data);
    ...
    axios.post("https://api.imgbb.com/1/upload?key=c1e87660595242c0175f82bb850d3e15", formData)
      .then(res => {
        ...
        if (url) {
            ...
            axios.patch(`${window.location.origin}/userUpdates/${user?.email}`, userCoverImage)
  ...
  const handleUploadProfileImage = e => {
    ...
    axios.post("https://api.imgbb.com/1/upload?key=c1e87660595242c0175f82bb850d3e15", formData)
      .then(res => {
        ...
        if (url) {
            axios.patch(`${window.location.origin}/userUpdates/${user?.email}`, userProfileImage)
            .then(res => console.log('Updated user cover image:', res))
           .catch(error => console.error('Error updating cover image:', error));
